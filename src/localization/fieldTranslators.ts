...
export const getFieldTranslationKeypaths = ({
  namespace,
  fieldGroup,
  fieldName,
  attribute,
}: FieldKeypathTProps & BaseFieldTProps): { local: string; global: string } => {
...
export const findDefault = (props: FieldTProps & BaseFieldTProps): string => {
  ...
  return (
    props.t.strict(translationPaths.local) ||
    props.t.default(translationPaths.global, props.fieldName)
  )
}
...
export const translateBlock = (props: BlockFieldProps & BaseFieldTProps) => {
  ...
  block.marks?.decorators?.forEach((decorator) => {
    const title =
      decorator.title ||
      props.t.strict(
        `${namespace}.fields.${fieldName}.block.marks.decorators.${decorator.value}`,
      ) ||
  ...
  block.marks?.annotations?.forEach((annotation) => {
    const title =
      annotation.title ||
      props.t.strict(
        `${namespace}.fields.${fieldName}.block.marks.annotations.${annotation.name}`,
      ) ||
