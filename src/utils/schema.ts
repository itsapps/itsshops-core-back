...
import { FieldContext, SchemaExtension } from '../types'
...
export function shapeSchema({
  ...
  const customGroups = extension
    ? (typeof extension.groups === 'function' ? extension.groups(ctx) : extension.groups) || []
    : []
  const customFieldsets = extension
    ? (typeof extension.fieldsets === 'function'
        ? extension.fieldsets(ctx)
        : extension.fieldsets) || []
    : []
  const customFields = extension
    ? (typeof extension.fields === 'function' ? extension.fields(ctx) : extension.fields) || []
    : []
  const fieldOverrides = extension
    ? (typeof extension.fieldOverrides === 'function'
        ? extension.fieldOverrides(ctx)
        : extension.fieldOverrides) || {}
    : {}
  const order = extension?.order || []
  const removeGroups = extension?.removeGroups || []

  const groupMap = new Map<string, FieldGroupDefinition>()
  const allGroups = [...coreGroups, ...customGroups]
