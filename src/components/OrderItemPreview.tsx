...
export default function OrderItemPreview(props: {
  item: OrderItem | OrderBundleItem
  orderId: string
}): React.ReactElement {
  ...
  const handleClick = async (documentType: string, id: string) => {
  ...
  const displayTitle =
    localizer.objectValue(variant, 'title') ||
  ...
  if (!isBundleProduct) {
    ...
    optionGroups = isBundleProduct
      ? []
      : (variantProps.options || []).map((option) => {
  ...
  let bundleItems: Array<{ count: number; title: string; key: string }> = []
  if (isBundleProduct) {
    const bundleProps = props.item as OrderBundleItem
    bundleItems = isBundleProduct
      ? (bundleProps.items || []).map((item) => {
          return {
            key: item.productId,
            count: item.count,
            title: localizer.objectValue(item, 'title') || 'No title',
          }
        })
      : []
  }
  ...
  const productButtonContent = (
    <Flex align={'center'} gap={2}>
      ...
      <Flex direction={'column'} gap={2}>
        ...
        {((product && product.sku) || (variant && variant.sku) || props.item.sku) && (
          <Text size={3} weight="bold">
            {props.item.sku || variant?.sku || product?.sku}
          </Text>
        )}
      </Flex>
    </Flex>
