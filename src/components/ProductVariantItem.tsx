  ...
  const getVariantTitles = (
    ...
    const optionTitles = variantOptions.map((option) => {
      const optionGroup = optionGroups.find((group) =>
        group.options.map((o) => o._id).includes(option._id),
      )
      if (!optionGroup) {
        return {
          group: null,
          ...
          <Flex gap={1} direction={['column', 'column', 'row']} justify={'space-between'}>
            <Stack space={1} flex={1}>
              <Flex flex={[1, 2, 3]} gap={1} direction={'column'}>
                <Heading as="h3" onClick={() => onClick(item._id)} style={{ cursor: 'pointer' }}>
                  {localizer.value(item.title)}
                </Heading>
                {item.sku && <Text size={0}>{item.sku}</Text>}
                <Container paddingTop={3}>
                  <ul style={{ margin: 0, marginLeft: '10px', padding: 0 }}>
                    {getVariantTitles(item.options, variantOptionGroups).map((entry) => (
                      // Use a unique ID or a combination of strings for the key instead of index
                      <li key={`${entry.group}-${entry.option}`}>
                        <strong style={{ fontWeight: 'bold' }}>{entry.group}</strong>:{' '}
                        {entry.option}
                      </li>
                    ))}
                  </ul>
