...
import { type FieldContext, type ITSContext, PRODUCT_TYPES, type ProductType } from '../../types'

export const createSharedProductFields = (
  ctx: FieldContext,
  type: ProductType,
): FieldDefinition[] => {
  const { f } = ctx
  ...
  const fields = [
    ...
    ...(vinofactEnabled && type !== PRODUCT_TYPES.BUNDLE
      ? [
...
export const createSharedProductGroups = (
  ctx: ITSContext,
  type: ProductType,
  ...
  return [
    ...
    { name: 'seo' },
    { name: 'vat' },
    ...(type === PRODUCT_TYPES.PRODUCT ? [{ name: 'variants' }] : []),
